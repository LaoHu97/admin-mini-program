...
var webSocket = {
  ...
  connectSocket: function (options) {
    ...
    socketClose = false
    socketMsgQueue = []
    let wsSessionId = wx.getStorageSync('wsSessionId')
    wx.connectSocket({
      url: api.websocketUrl + '/?auth_token=' + wsSessionId + '&transport=websocket',
      header: {
        io: wsSessionId
      },
  ...
  wxTextToSpeech(res) {
    ...
    plugin.textToSpeech({
      ...
      content: res,
      success: function (res) {
        console.log("播报语音地址：", res.filename)
        webSocket.playWechatSI(res.filename, r)
      },
      fail: function (res) {
        console.log("播报语音错误", res)
      }
    })
  },
...
wx.onSocketOpen(function (res) {
  ...
  if (socketClose) {
    ...
    webSocket.sendSocketMessage(para)
